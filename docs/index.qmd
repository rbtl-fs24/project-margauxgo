...
Lastly, it is interesting to have a look at the data collected about the willingness to separate plastics at home before collection. Indeed, if household plastics are collected, sorted and processed correctly, they can be recycled two, three, four or more times [@recycling]. For this, we chose to create a horizontal bar chart.

```{r, message=FALSE, warning=FALSE, include=TRUE}
#| label: fig-willingness
#| fig-cap: "Willingness to separate plastics at home"
...
# Save data underlying the figure ---------------------------
write_csv(long_table, 
          here::here("data/final/fig-willingness.csv"))
```

As displayed in @fig-willingness, the majority of the population is willing to separate plastics at home before collection. However, the lack of space comes first for a reason to not to so, followed by the lack of time.

# Conclusions

The findings of our study can be summarised in three major points.
