...
As shown in @tbl-recycle-rate, the plastic recycling rate is 95.2%, indicating that almost every participant recycles plastic. This is a promising start. But which type of plastic is recycled the most? Does everyone recycle all types of plastic? To answer this, we choose to create a stacked bar chart displaying the recycling rates for each type of plastic.
...
Finally, we examine the willingness to separate plastics at home before collection. Properly collected and sorted household plastics can undergo recycling multiple times [@recycling], highlighting the importance of this practice. Our data presentation choice is a horizontal bar chart.
...
In @fig-willingness, we observe that a majority of the population is willing to separate plastics at home before collection. However, space constraints are cited as the primary reason for not doing so, followed by time constraints.
